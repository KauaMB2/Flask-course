...
class User(db.Model, UserMixin):
	...
	def can_purchase(self,item_obj_price):
		return self.budget >= item_obj_price
...
class Item(db.Model):  # Cria classe
	...
	comm = db.Column(db.String(length=1024),nullable=False, unique=False)
	def __repr__(self):  # (????????????????????????????????????????????????)
